...
def inicializar_rag(diretorio_rag: str = r'.\RAG') -> tuple[list, faiss.IndexFlatL2]:
    ...
    for index, doc in enumerate(documentos):
        if index % 250 == 0:  # Reduzido para feedback mais frequente
            print(f"Gerando embedding {index}/{len(documentos)}...")
        try:
            response = ollama.embed(model="nomic-embed-text", input=doc)
            embeddings.append(response['embeddings'][0])
        except Exception as e:
            print(f"Erro ao gerar embedding para documento {index}: {e}")
            embeddings.append(np.zeros(768))  # Placeholder para evitar falhas (ajuste a dimensão se necessário)
...
def buscar_contexto(pergunta: str, documentos: list, index: faiss.IndexFlatL2, k: int = 5) -> list:
...
def testar_contexto():
    ...
    while True:
        ...
        contextos = buscar_contexto(frase, documentos, index, k=5)
